...
This fork is maintained for study, experimentation, and contribution planning around the RLax codebase.  
RLax is a research-focused library, and most high-impact contributions are incremental improvements to documentation, tests, and developer clarity rather than large feature additions.

The goal is to better understand RLax internals, follow DeepMind JAX ecosystem conventions, and prepare high-quality upstream contributions.

---

## 🚀 Best First Contribution Ideas (Low Risk, High Acceptance)

These contribution ideas are intentionally scoped to be small, review-friendly, and aligned with upstream maintenance priorities.

### 🟢 Option A (Strongly Recommended)
**Improve documentation for deprecated APIs**
- Add explicit notes such as: *“Deprecated – consider using `distrax` instead”*
- No functional or behavioral changes
- Very high likelihood of acceptance
- Ideal first PR to establish contributor trust

### 🟢 Option B
**Add clarifying comments in tests**
- Focus on files such as `rlax/_src/distributions_test.py`
- Explain why multiple test variants exist (e.g. `jit`, `vmap`, device placement)
- Improves readability for new contributors without affecting test logic

### 🟢 Option C
**Improve README or developer documentation**
- Clarify test exclusions (e.g. hardware-specific behavior)
- Mention known macOS / local CPU differences
- Add brief guidance on running tests locally with limited devices

---

## 🧪 Local Development Notes

- RLax tests are designed to validate correctness under:
  - eager execution
  - `jax.jit` compilation
  - `jax.vmap` batching
- Some tests (e.g. `pop_art_test`) may fail locally due to device or threading constraints.
- This behavior is consistent with upstream CI assumptions.

Example local test command:

```sh
pytest --numprocesses auto --pyargs rlax -k "not pop_art_test"
